...
from pazar.models import Skill, Free_Listing, Customer
...
class ProfileView(LoginRequiredMixin, DetailView):
    ...
    # def get_context_data(self, **kwargs):
    #     kwargs['skills'] = Skill.objects.filter(user=self.request.user)
    #     return super().get_context_data(**kwargs)
...
#         like, created = PostLike.objects.get_or_create(liked_by=user, post_id=post_id)
...
#         liked = PostLike.objects.filter(liked_by=user, post=post).count()
#         if user in post.postlike_set.all():
#             post.postlike_set.remove(user)
#         else:
#             pass
#         like, created = PostLike.objects.get_or_create(liked_by=user, post=post)
#         # if not liked:
#         #     like = PostLike.objects.create(liked_by=user, post=post)
#         #     like.value = 'Like'
#         #     current_likes = current_likes + 1
#         # else:
#         #     PostLike.objects.filter(liked_by=user, post=post).delete()
#         #     current_likes = current_likes - 1
#         if not created:
#             if like.value == 'Like':
#                 like.value = 'Unlike'
...
def post_comment_create_and_list_view(request):
    ...
    for p1 in qs:
        ...
        liked_user = PostLike.objects.filter(post=p1).order_by('-cr_date')[0:4]
    ...
    if 'submit_p_form' in request.POST:
        ...
        if post_form.is_valid():
            ...
            post_form.instance.uploaded_by = request.user
            post.save()
            post_form.save_m2m()
            images = request.FILES.getlist('pic')
            for i in images:
                PostFileContent.objects.create(uploaded_by=request.user, post=post, file=i)
            post_form = PostForm()
